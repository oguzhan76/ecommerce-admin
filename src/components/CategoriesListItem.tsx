            ...
            {editing ?
                <div key={item._id} className='rounded-lg bg-slate-200 px-2 inline-flex justify-between h-16 items-center'>
                    <form onSubmit={saveEdit} className='flex gap-3'>
                        ...
                        <CategoryParentDropdown categories={categories} setParent={setNewParentId} defaultParentName={item.parent} />
                ...
                <>
                    ...
                    {showSubs && (<div className='pl-6 flex flex-col gap-1 border-l-2 border-dotted'>
                        {subs.map(item =>
                            <CategoriesListItem
                                key={item._id}
                                item={item}
